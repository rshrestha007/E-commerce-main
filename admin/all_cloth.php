...
else
{
 ...
 if(isset($_POST['update_update_btn'])){
  ...
  $update_quantity_query = mysqli_query($conn, "UPDATE `cloth` SET quantity = '$quantity' , name='$name' , catagory='$catagory' ,price='$price'  WHERE id = '$update_id'");
  if($update_quantity_query){
     header('location:all_cloth.php');
  };
};

 if(isset($_GET['remove'])){
  $remove_id = $_GET['remove'];
  mysqli_query($conn, "DELETE FROM `cloth` WHERE id = '$remove_id'");
  header('location:all_cloth.php');
};
?>
<!DOCTYPE html>
  ...
  <?php
          if (mysqli_num_rows($result) > 0) {
            ...
            while($row = mysqli_fetch_assoc($result)) {
              ?>
    <tr>
      <td><img src="cloth_img/<?php echo $row['imgname']; ?>" style="width:50px;"></td>
     <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
      ...
      <td><a href="all_cloth.php?remove=<?php echo $row['id']; ?>">remove</a></td>
    </tr>
    <?php 
    }
        } 
